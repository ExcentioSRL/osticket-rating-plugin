...
if (isset($_GET["export"])) {
    ...
    if (!empty($items)) {
        foreach ($items as $item) {
            if (!$heading) {
                fputcsv($csv,explode("\t", "Date\tTicket\tTopic\tOperator\tRating\tUser\tUser IP\tReferrals\tCustomer Experience" . "\n"));
                $heading = true;
            }
            $tmpRow = ($item['timestamp'] != "" ? date("d/m/Y H:i:s", strtotime($item['timestamp'])) : "-" ). "\t" . $item["number"] . "\t" . $item["topic"] . "\t" . $item["username"] . "\t" . $item["rating"] ."\t". $item["name"] . "\t" . $item["user_ip"] . "\t". formatReferrals($item['referrals'], $item['referrals_dept'], $item['referrals_team']) . "\t" . $item["c_experience"]."\n";
